...
//			02/21/17	modified for session 04 by Alec Daling
//
//  Notes:
...
main ()
{
  ...
  //cin >> dimension;
	...
	//open file output stream to record duration of calculations
	ofstream out ("eigen_times.dat");
	
	for(int dimension = 50; dimension <= 1000; dimension+=20)
	{ //open dimension loop
		...
		out << log10(dimension) << "	" << log10((double) (end - start) / (double) CLOCKS_PER_SEC) << endl;
