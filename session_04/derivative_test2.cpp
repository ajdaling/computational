...
main (void)
{
  void *params_ptr;		// void pointer passed to functions 
	params_struct my_params; //parameters structure to hold alpha and beta
	
  const double hmin = 1.e-10;	// minimum mesh size 
  double x = 2.;		// find the derivative at x 
  double alpha = 3.;		// a parameter for the function 
  double beta = 3./2.; // b parameter for function
  ...
  ofstream out ("derivative_test2.dat");	// open the output file 

	my_params.alpha = alpha; //load structure with values
	my_params.beta = beta;
  params_ptr = &my_params;		// struct pointer to be passed to functions 
...
{
	params_struct *passed_params;
	passed_params = (params_struct *) params_ptr;
	
  double alpha = passed_params->alpha;
	double beta = passed_params->beta;
